...
// these exist in this ogl version
...
void main() {
	...
	vec3 worldpos = playerpos;
		...
		if(	
			...
			worldpos += calcMovePlants(playerpos + cameraPosition) / 255.0 * max(gl_MultiTexCoord1.y,0.5);
			...
			if(blockId == BLOCK_AIR_WAVING) worldpos = playerpos + calcMoveLeaves(playerpos + cameraPosition, 0.0040, 0.0064, 0.0043, 0.0035, 0.0037, 0.0041, vec3(1.0,0.2,1.0), vec3(0.5,0.1,0.5))/255.0*gl_MultiTexCoord1.y;
